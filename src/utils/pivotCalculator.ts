export class PivotCalculator {
  ...
  constructor(type = 'Traditional', timeframe = 'Auto', numberPivotsBack = 15) {
    ...
    this.timeframe = timeframe === 'Auto' ? 'Daily' : (timeframe || 'Daily');
    this.numberPivotsBack = Math.max(1, parseInt(String(numberPivotsBack)) || 15);
  }
    ...
    try {
      ...
      for (let i = 0; i < candles.length; i++) {
        ...
        const currentDate = new Date(
          currentCandle.timestamp || currentCandle.time * 1000 || currentCandle.time
        );
        ...
        if (i === 0) {
          isNewPeriod = true;
          currentPeriodHigh  = currentCandle.high  || 0;
          currentPeriodLow   = currentCandle.low   || 0;
          currentPeriodClose = currentCandle.close || 0;
        } else {
          const prevCandle = candles[i - 1];
          const prevDate   = new Date(
            prevCandle.timestamp || prevCandle.time * 1000 || prevCandle.time
          );
        ...
        if (isNewPeriod && i > 0) {
          ...
          currentPeriodHigh  = Math.max(currentPeriodHigh,  currentCandle.high  || 0);
          currentPeriodLow   = Math.min(currentPeriodLow,   currentCandle.low   || 0);
          currentPeriodClose = currentCandle.close || 0;
        }
  ...
  calculatePivots(H: number, L: number, C: number) {
    let P: number, R1: number, S1: number, R2: number, S2: number;
    let R3: number, S3: number, R4: number, S4: number, R5: number, S5: number;
    ...
    if (this.type === 'Fibonacci') {
      ...
      R4 = P + 1.382 * range;  S4 = P - 1.382 * range;
      R5 = P + 1.618 * range;  S5 = P - 1.618 * range;
  ...
  isNewAnchor(currentDate: Date, prevDate: Date): boolean {
    ...
    if (anchor === 'weekly') {
      ...
      const curDay  = currentDate.getDay() === 0 ? 7 : currentDate.getDay();
