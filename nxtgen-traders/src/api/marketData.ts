...
// DataLoader-style batching for Yahoo Finance
let batchQueue: { symbol: string, exchange: string, resolve: (q: Quote | null) => void }[] = [];
...
async function processBatch() {
  ...
  // Yahoo allows up to 200 symbols per request, we just join them
  const symbols = currentQueue.map(q => q.exchange === 'NSE' ? `${q.symbol}.NS` : `${q.symbol}.BO`).join(',');
  ...
  try {
    ...
    for (const req of currentQueue) {
      const yfSymbol = req.exchange === 'NSE' ? `${req.symbol}.NS` : `${req.symbol}.BO`;
      const quote = results.find((r: any) => r.symbol === yfSymbol);
      if (quote) {
        ...
        req.resolve({
          symbol: req.symbol,
          exchange: req.exchange as 'NSE' | 'BSE',
          ltp,
          open: quote.regularMarketOpen || close,
          high: quote.regularMarketDayHigh || ltp,
          low: quote.regularMarketDayLow || ltp,
          close,
          change: parseFloat(change.toFixed(2)),
          changePct: parseFloat(((change / close) * 100).toFixed(2)),
    ...
    currentQueue.forEach(req => req.resolve(null));
  }
}

async function fetchFromYahooFinance(symbol: string, exchange: 'NSE' | 'BSE'): Promise<Quote | null> {
  return new Promise((resolve) => {
    batchQueue.push({ symbol, exchange, resolve });
    if (!batchTimeout) {
...
export async function fetchQuote(symbol: string, exchange: 'NSE' | 'BSE' = 'NSE'): Promise<Quote> {
  ...
  // Fallback chain: Yahoo Finance → Twelve Data → Alpha Vantage → stale mock
  let quote = await fetchFromYahooFinance(symbol, exchange);
  if (!quote) quote = await fetchFromTwelveData(symbol, exchange);
...
async function fetchOHLCVFromYahooFinance(
  symbol: string,
  exchange: 'NSE' | 'BSE',
  timeframe: Timeframe,
  bars: number
  ...
  try {
    const yfSymbol = exchange === 'NSE' ? `${symbol}.NS` : `${symbol}.BO`;
    ...
    let range = '1y';
    if (timeframe === '1m' || timeframe === '5m') range = '5d';
    else if (timeframe === '15m' || timeframe === '30m' || timeframe === '1h') range = '1mo';
    
    const targetUrl = encodeURIComponent(`https://query1.finance.yahoo.com/v8/finance/chart/${yfSymbol}?interval=${interval}&range=${range}`);
    const res = await fetch(`https://api.allorigins.win/get?url=${targetUrl}`);
    if (!res.ok) return null;
    ...
    if (!raw.chart || !raw.chart.result || raw.chart.result.length === 0) return null;
    ...
    console.error('Yahoo Finance OHLCV fetch failed', error);
    return null;
  }
}
...
export async function fetchOHLCV(
  ...
  // Try Yahoo Finance
  const yfData = await fetchOHLCVFromYahooFinance(symbol, exchange, timeframe, bars);
